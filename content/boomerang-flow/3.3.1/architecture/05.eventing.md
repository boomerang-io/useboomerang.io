...
Boomerang Flow supports events through an eventing implementation based on CloudEvents and HTTPS/NATS. You can also optionally choose to extend this with other eventing systems, like Azure<sup>®</sup> Event Grid and KNative<sup>®</sup> eventing, for example.
...
| `token` | The access token can be provided as a query parameter or provided in the Authorization header. See the [Security Architecture](/boomerang-flow/docs/architecture/security) to know more. |
